      ...
      setTimeout(_ => {
        ...
        this.setData({
          imgList: [...this.data.imgList, ...d.items.map(v => {
            ...
            v.img_url = v.url.replace('1920x1080', '640x480');
            v.img_title = v.title ? v.title : v.copyright;
            return v;
          })],
  ...
  handleClickGenerateShareImg(imgInfo) {
    writePhotosAlbum().then(_ => {
      ...
      const map = {
        ...
        // },
        date: {
          text: dayjs(imgInfo.date).format('DD MMM.YYYY'),
          x: 15,
          y: defaultTextY + 30,
          fontSize: 16,
          align: "left"
      ...
      Promise.all([downloadImage(bg)]).then(([bg]) => {
